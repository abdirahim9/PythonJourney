...
class Player:
    """Manages player stats, file I/O, guess history with Pandas DataFrame analysis and visualization."""
    def __init__(self):
    ...
    def get_guesses_df(self):
        """Get Pandas DF from history (reusable for efficiency)."""
    ...
    def analyze_guesses(self):
        ...
        sorted_guesses = np.sort(df['guesses'].values)  # Hybrid with NumPy for sort
        trend = df['guesses'].rolling(window=5).mean().iloc[-1] if len(df) >= 5 else "Insufficient data"

        # Safely format trend
        trend_str = f"{trend:.2f}" if isinstance(trend, (float, np.floating)) else str(trend)

        return (
            f"Mean: {mean:.2f}, Std Dev: {std_dev:.2f}, Sorted: {sorted_guesses}, "
...
class Game:
    """Handles guessing game logic with Pandas-integrated stats and viz."""
...
def main():
    """Main game loop with final Pandas analysis, CSV export, and histogram plot."""
    player = Player()
    player.load_stats()
    threads = []
    play_again = "y"
