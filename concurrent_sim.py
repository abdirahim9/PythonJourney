...
class Signal:
    ...
    def __init__(self, length=10, frequency=5, scale=1.0, data=None):
        self.length = length
    ...
    def generate(self):
        ...
        return (base + pattern) * self.scale
    ...
    def load_state(self, filename="sim_state.json"):
        ...
        try:
            with open(filename, 'r') as f:
                state = json.load(f)
            with self.lock:
                self.signals = [Signal(length=s['length'], frequency=s['frequency'], scale=s['scale'], data=s['data']) for s in state]
...
def concurrent_generate(sim, length=20, scale=1.0):
    signal = Signal(length=length, scale=scale)
...
def main():
    ...
    for _ in range(4):  # 4 concurrent
        t = threading.Thread(target=concurrent_generate, args=(sim, 20, scale))
        threads.append(t)
        t.start()
    for t in threads:
        t.join()
    print("Concurrent Signals Added.")
